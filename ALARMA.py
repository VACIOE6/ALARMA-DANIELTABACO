...
texto1 = Label(ventana, text= 'Horas', bg= 'white', fg= 'black', font=('Arial',12, 'bold'))
texto1.grid(row=1, column=0, padx=5, pady=5 )
texto2 = Label(ventana, text= 'Minutos', bg= 'white', fg= 'black', font=('Arial',12, 'bold'))
texto2.grid(row=1, column=1, padx=5, pady=5 )
texto3 = Label(ventana, text= 'Segundos', bg= 'white', fg= 'black', font=('Arial',12, 'bold'))
...
style = ttk.Style()
style.theme_create('combostyle', parent= 'alt', settings= {'TCombobox':{'configure':{'selectbackground': 'red','fideldbackground': 'gold', 'background':'blue'}}})
...
ventana.option_add('*TCombobox*Listbox*selectForeground', 'blue')

alarma = Label(ventana, fg='blue2', bg='white', font=('Radioland',20))
alarma.grid(column=0, row=3, sticky="nsew", ipadx=5, ipady=20)
repetir = Label(ventana, fg= 'black', bg='white', text='Repetir:', font='Arial')
    ...
    if int(hora) == int(x_hora):
        if int(minutos) == int(x_minutos):
            if int(segundos) == int(x_segundos):
                mixer.music.load("Trompeta Militar.mp3")
...
texto_hora = Label(ventana, fg= 'blue2', bg='white')
texto_hora.grid(columnspan=3, row=0, sticky="nsew", ipadx=5, ipady=20)

botonStopAlarma = Button(ventana, text="Set Alarma",bg="#e2504c",fg="#fbfbfb",font=("Roboto", 15, "bold"), width=10, height=1, command=botonStopAlarma)
botonStopAlarma.grid(column=1, row=4, ipadx=10, ipady=20)
